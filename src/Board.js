...
export default class Board extends React.Component {
  ...
  initializeDragula() {
    // Specify the containers where the cards will be drag-and-drop
    const containers = [
    ...
    this.drake.on("drop", (el, target) => {
      ...
      const targetLane = target.getAttribute("id");
      // console.log(targetLane);
      this.updateCardClassName(cardId, targetLane);
    });
  }

  //  new function to update the card's class name
  updateCardClassName(cardId, targetLane) {
    const cardElement = document.querySelector(`[data-id="${cardId}"]`);
    console.log("cardId" + cardId);
    console.log("cardElement" + cardElement);
    console.log("targetLane: " + targetLane);
    if (cardElement) {
      cardElement.className = "Card"; // Reset the class
      if (targetLane === "backlog") {
        cardElement.classList.add("Card-grey");
      } else if (targetLane === "inProgress") {
        cardElement.classList.add("Card-blue");
      } else if (targetLane === "complete") {
